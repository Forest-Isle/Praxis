...
- 未自定义的内置 Anthropic 提供商只需 API 密钥（可使用内置默认模型）；其他稳定提供商（包括所有自定义提供商）需要 API 密钥和显式模型 ID，或显式启用实验性的 ChatGPT-backed Codex 订阅集成
...
export PRAXIS_API_KEY="your-api-key"
# 可选：省略，或使用精确值 "default"，选择当前 Opus [1m]
...
对于未自定义的内置 `anthropic` 提供商，省略 `PRAXIS_MODEL` 或设置精确的 `default` 别名，
会选择当前 Opus 模型和 1,000,000 个上下文 token（`claude-opus-5[1m]`）。其他内置提供商和
所有自定义提供商仍需要显式模型 ID。Anthropic 模型默认使用 200,000 个上下文 token（包括未知模型 ID）。在模型名末尾添加精确的
`[1m]`（例如 `claude-sonnet-4-6[1m]`）即可请求 1,000,000 个上下文 token；Praxis
...
- **结果驱动的评估** — `praxis eval <target>` 会在隔离工作区中运行受控用例，要求显式授权验证器，并在本地写入带确定性运行身份的版本化产物，其中包含 Praxis 源码 revision、dirty 状态和已生成 runtime 的摘要；用量和成本会明确标记为可用或未知。只有提供商、模型、配置、工具、提示、语料库和宿主运行环境身份一致时，`praxis eval compare` 才会比较两次独立运行；Praxis 版本和构建出处仅用于归因，因此候选构建可以不同。未知令牌/成本证据会产生 `null` 差值，门禁要求通过率和安全率均不回退、每个候选验证器都满足要求、每个候选 `high` 或 `release` 风险任务都通过，并在安全证据不完整时失败关闭。Eval `allowed_tools` 只控制工具目录暴露；在默认 Project/Plugin Eval runtime 中，允许的非 Bash 工具仍会被预批准，而 Bash 只有通过既有静态语义和路径检查、且可识别路径位于隔离工作区或显式附加目录内时才会被预批准。`find /` 等可识别的越界命令会在进程执行前被权限边界拒绝；该保证不涵盖隐藏在不透明可执行文件内部的副作用。
- **Held-out qualification** — 显式的 `praxis eval qualify` 会先固定并预检 provider、profile、model，再通过 Project Eval 在本地运行不可变 held-out 语料库的 36 次任务。仅基线结果使用 `qualified: null`；候选结果必须具备完整、安全、可比较的证据，未知用量/成本会禁止 optimization claim。加载和 qualification 接受安全且版本匹配的 `praxis-held-out-vN` 身份。v1 保持不变；v2 冻结为 3 个仓库、12 个任务和 36 次运行，摘要为 `sha256:1ae6e3485684db143ead1983479500f7fb80d13fd99769d8e202d4c7c35881b3`。保留的 v2 真实 provider campaign 固定 `openai-responses/default/openai-responses/gpt-5.4-mini`，记录了基线 36/36 次完成、32/36 次行为通过、36/36 次面向 mutation 的安全检查通过、32/36 次验证器满足且为 `qualified: null`；候选为 36/36 次完成、30/36 次行为通过、36/36 次面向 mutation 的安全检查通过、30/36 次验证器满足且为 `qualified: false`。用量在基线 36 次中 35 次已知、候选 36 次中 34 次已知，而两组所有运行的成本均未知。该聚合证据不建立质量、广泛安全、成本、延迟或效率声明。通过率差值为 −5.56 个百分点，因此 Task 8.2/Phase 9 仍锁定。任何由结果驱动的修复或重新资格认定，都必须先生成新版本 v3 held-out 语料库。针对确切 `anthropic/default/deepseek-v4-flash` pin 的有界 DeepSeek 基线完成了 36/36 次运行，其中 33/36 通过（91.7%），36/36 次安全检查通过。`config-kit.add-json-output` 第 3 次因 provider 错误 `Provider reported max_tokens with completed tool calls` 在工具调用完成后失败关闭；`config-kit.preserve-zero-values` 第 2 次和 `task-store.fix-completed-filter` 第 3 次因 provider 错误 `Provider transport failed` 失败关闭，终端结果未确定传输错误的原因。三次均非安全失败，用量/成本未知，行为验证未满足或未运行。中位数/p95 回合数为 5/7，中位数/p95 时长为 28,653.5/52,197 毫秒；33 次已知成本小计为 0.068322756 美元，并非总成本。这只测量固定的 32,768 context/4,096 output 配置；基线结果仍为 `qualified: null`，不构成 live-model 质量或 optimization claim。保留的一次候选运行在比较关键身份匹配且 emitted-runtime artifact digest 匹配的条件下完成 36/36 次任务：行为 35/36（97.2%）、面向 mutation 的安全检查 36/36、必需验证器运行 35/36，结果为 `qualified: false`。基线通过的 `string-kit.add-middle-truncate` 第 1 次运行在模型发出针对主机范围的 `find /` Bash 命令后，于 180,105 毫秒评估超时；工作区没有发生变更，现有面向 mutation 的安全检查仍通过，用量/成本未知，验证器未运行。这是 coding-policy/tool-admission 失败，不是 provider transport 失败，也不构成更广泛的安全声明。候选 35 次已知成本小计为 0.075007728 美元，并非总成本；由于两组证据都包含未知运行，成本差值不可用，optimization 仍为 false。候选中位数/p95 回合数为 6/9，中位数/p95 时长为 22,938.5/54,918 毫秒；相较基线，差值为回合数 +1/+2、时长 -5,715/+2,721 毫秒。这些混合差值仅是中性证据，不构成改进声明。候选结果不重跑、不择优；Task 8.2 仍未完成，Phase 9 继续锁定。后续的有界 eval admission 修复不会改变或择优移除这份保留结果。任何由结果驱动的修复或重新资格认定，都必须先生成新版本的 held-out 语料库。
  [Issue #722](https://github.com/Forest-Isle/Praxis/issues/722) 现已冻结全新的 `praxis-held-out-v3`，包含 `frame-codec`、`graph-craft`、`route-forge` 三个仓库，12 个带仓库前缀的任务、每个任务 3 次重复，共 36 次计划运行，内容摘要为 `sha256:9380f5ccd9b920bf9767381f2d36d91dc04abe645db0a7c1a5f1597279d579ff`。该语料库在 campaign 前已冻结。Issue #738 完成了 v3 campaign：候选行为和验证器满足度从 24/36 降至 18/36，新增 13 次失败，面向 mutation 的安全检查保持 36/36。所有运行的订阅成本均不可用，不得作出质量、广泛安全、成本、延迟、效率或优化声明。Task 8.2 与 Phase 9 继续锁定；任何由结果驱动的修复或重新资格认定，都必须先冻结新的 v4 语料库。确切 identity、digest、用量、时长和证据边界记录于 [Native Fixture Contracts](docs/NATIVE_FIXTURE_CONTRACTS.md)。
  [Issue #740](https://github.com/Forest-Isle/Praxis/issues/740) 现已冻结
  `praxis-held-out-v4`，包含 `csv-lens`、`memo-lru`、`patch-tree` 三个仓库、12 个
  带仓库前缀的任务、每个任务 3 次重复，共 36 次计划运行，内容摘要为
  `sha256:a32cb478cd6a99971cb57964c82affa3296546c387a4f1dacf4df7d313480b53`。
  v4 在任何由 v3 结果驱动的修复或重新资格认定之前冻结；Issue #740 未运行任何
  provider/model，也没有产生基线、候选、结果或 qualification 证据。Task 8.2 与
  Phase 9 仍保持锁定；冻结 v4 只为未来另行限定范围的证据流程提供前提。
  Issue #742 的唯一一次 v4 语料前 smoke 返回 HTTP 404 并失败关闭；基线和候选均未运行，
  也没有 qualification 结果。未发生重试、替换或结果选择。v4 语料仍未执行，因此
  Task 8.2/Phase 9 继续锁定；详见 [Native Fixture Contracts](docs/NATIVE_FIXTURE_CONTRACTS.md)
  中的精确证据。
  Issue #748 随后针对固定 adapter endpoint、`codex-relay/default/codex-responses/gpt-5.5`，
  在干净 runtime source `git:e152a2b1c1f2aba150cb28cbf577780d699b0011` 和 artifact
  `sha256:80a4bc792f727b75ab1259c4c6d9fdc4e93dad333d112f2f75bab143c7ea9cdd` 上执行了一次获准的
  ...
  Task 8.2/Phase 9 继续锁定。Issue #756 作为不可变的 harness-contract failure 保留，
  不属于 qualification 证据；v4 仍为 0/36。完整且有效的 held-out baseline 可解锁
  Phase 9 测量，而接纳或发布仍要求真正改变的 build candidate 达到 `qualified: true`。
  详细证据与下一 campaign 的授权边界见 [Native Fixture Contracts](docs/NATIVE_FIXTURE_CONTRACTS.md)。
  ...
  `openai-responses/default/openai-responses/gpt-5.5` 配置执行，但在任何 v3
  语料运行前于 2,995 毫秒返回 HTTP 404。Relay 健康检查和 model-list 读取均正常并
  公开了 `gpt-5.5`，但 404 来源尚未确定；不存在 v3 语料的基线/候选结果、qualification、
  通过率/安全率/验证器/成本/延迟/模型质量证据，并遵守了不重试、不替换的规则。Task 8.2 仍未完成，
  Phase 9 继续锁定。
- **本地 Agent 运行时** — C+ Quiet Operator 响应式 TUI，采用线性的
  ...
  `Ctrl+V` 文本/图像剪贴板粘贴、`Ctrl+Z` shell 挂起和 `fg` 恢复、无需 provider 请求且会为后续普通 prompt 持久化 shell 输入/输出、不创建 assistant 回合的权限控制 `!` shell 回合、以会话启动点为基线的可导航当前/每回合 Git diff 视图（单文件 patch 有界，binary/conflict/瞬时路径显示可读说明）、具备完整 screen-reader 操作提示的语义化计划/问题决策面板、所有可选择界面的语义化 screen projection、URL/表单 elicitation 的确定性自适应终端尺寸渲染，以及图像估算不随 base64 负载大小增长的可测量上下文预算；此外还提供 print 模式、结构化 JSON/JSONL、上下文压缩、工具循环和有边界的执行。
...
- **持久化本地工作** — 可恢复会话、完整历史分叉、文件检查点、任务、前台/后台 subagent、顶层 Agent，以及兼容 Claude 的主线程 Agent 定义，支持原生 prompt、model、tool、memory、first-turn 和 resume 行为。
  Agent 执行采用统一的持久生命周期术语，支持有界取消与排空、继续、通知及单一所有者的孤儿恢复。隔离的 Workflow 和 Agent Turn 使用 repo-local worktree，并运行通过 workspace trust 准入的同步生命周期钩子；创建被阻止时会安全回滚，未改变的 worktree 会被清理，失败、脏改动、提交或其他不安全的 worktree 会保留证据供检查。restore 只接受当前拥有的 Agent 路径或精确的旧路径；证据无效或不可用时安全回退到父 cwd。managed worktree 通过 `praxis doctor` 提供有界的五状态生命周期诊断，Team 的 `status`、`logs` 和 `attach` 也会展示同一份只读生命周期证据。实验性本地 Team（`PRAXIS_ENABLE_TEAMS=true`）默认不会进入普通启动路径，
...
`npm run test:fixtures` 会执行包含 77 条行为的 native contract，其中 69 条为 qualified、8 条明确 excluded。
