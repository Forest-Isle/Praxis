...
export type ClaudeInterruptionClassification =
  ...
  | {
      ...
      prompt?: string
      replayEntries?: NativeTranscriptEntry[]
...
function plainUserPrompt(entry: NativeTranscriptEntry): string | undefined {
  if (
    entry.type !== 'user' ||
    entry.isMeta === true ||
...
function assistantHasVisibleText(entry: NativeTranscriptEntry): boolean {
...
function terminalToolResult(entry: NativeTranscriptEntry): boolean {
