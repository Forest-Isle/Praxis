...
function advancesLogicalTail(entry: NativeTranscriptEntry): boolean {
...
function validateNativeHistory(
  ...
  const entriesByUuid = new Map<string, NativeTranscriptEntry>()
  ...
  let compactBoundary: NativeTranscriptEntry | undefined
...
export function createClaudeNativeFork({
  ...
  sessionId,
  resumeSessionAt,
}: ClaudeNativeForkOptions): NativeTranscriptEntry[] {
  const agentColors: NativeTranscriptEntry[] = []
  const titles: NativeTranscriptEntry[] = []
  const modes: NativeTranscriptEntry[] = []
  const permissionModes: NativeTranscriptEntry[] = []
  const prLinks: NativeTranscriptEntry[] = []
  const history: NativeTranscriptEntry[] = []
  const nativeHistory: NativeTranscriptEntry[] = []
  let lastPrompt: NativeTranscriptEntry | undefined
  let nativeLastPrompt: NativeTranscriptEntry | undefined
