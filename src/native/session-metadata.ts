...
export function reduceClaudeSessionMetadata(
  ...
  const latest = new Map<ClaudeDurableMetadataType, NativeTranscriptEntry>()
  ...
  if (lastPrompt !== undefined) {
    ...
    if (trustedCommittedLeafUuid === undefined) {
      ...
      if (localCommandLeaf === undefined) {
        ...
        const snapshot: NativeTranscriptEntry = {
          ...lastPrompt,
...
export function mergeClaudeDurableMetadataSnapshot(
  baseline: readonly NativeTranscriptEntry[],
  observed: readonly NativeTranscriptEntry[],
  sessionId: string,
): NativeTranscriptEntry[] {
