...
export interface NativeScheduledTask {
  id: string
  cron: string
  prompt: string
  createdAt: number
...
interface ScheduledTaskDocument {
  tasks: NativeScheduledTask[]
  [key: string]: unknown
}

export interface NativeScheduledTaskStoreOptions {
  filePath: string
  lockFile?: string
}

export type NativeScheduledTaskCreateInput = Pick<
  NativeScheduledTask,
  | 'cron'
  | 'prompt'
  | 'createdAt'
...
export interface NativeScheduledTaskCreateOptions {
  maxJobs?: number
}

export class NativeScheduledTaskLimitError extends Error {
  constructor(readonly maxJobs: number) {
    ...
    this.name = 'NativeScheduledTaskLimitError'
  }
}
...
function parseTask(value: unknown): NativeScheduledTask {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new Error('Invalid native scheduled task')
  }
  ...
  return { ...task } as NativeScheduledTask
}
...
function parseDocument(
  ...
  try {
    ...
    throw new Error(`Invalid native scheduled task JSON: ${filePath}`, {
      cause: error,
    })
  }
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw new Error(`Invalid native scheduled task document: ${filePath}`)
  }
  ...
  if (new Set(tasks.map(({ id }) => id)).size !== tasks.length) {
    throw new Error(`Duplicate native scheduled task ID: ${filePath}`)
  }
  return { ...document, tasks }
}

export class NativeScheduledTaskStore {
  private readonly filePath: string
  private readonly lease: ExclusiveFileLease

  constructor(options: NativeScheduledTaskStoreOptions) {
  ...
  async list(): Promise<NativeScheduledTask[]> {
    return (await this.readRecord()).document.tasks.map((task) => ({ ...task }))
  }

  async create(
    input: NativeScheduledTaskCreateInput,
    options: NativeScheduledTaskCreateOptions = {},
  ): Promise<NativeScheduledTask> {
    return this.withLock(async () => {
      for (let attempt = 0; attempt < MAX_MUTATION_RETRIES; attempt += 1) {
        ...
        if (
          ...
          throw new NativeScheduledTaskLimitError(options.maxJobs)
        }
        const ids = new Set(document.tasks.map(({ id }) => id))
        let id = randomBytes(4).toString('hex')
        while (ids.has(id)) id = randomBytes(4).toString('hex')
        const task: NativeScheduledTask = { id, ...input }
        if (
      ...
      throw new Error(`Native scheduled task file changed too frequently`)
    })
  }
  ...
  private async withLock<T>(operation: () => Promise<T>): Promise<T> {
    ...
    while (!lock) {
      if (Date.now() - started >= LOCK_WAIT_MS) {
        throw new Error(
          `Native scheduled task store is locked: ${this.filePath}`,
        )
      }
