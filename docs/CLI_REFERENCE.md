...
authorized comparison. The one preserved candidate used matching
comparison-critical identities and a matching emitted-runtime artifact digest:
36/36 runs completed, with 35/36 behavior (97.2%), 36/36 mutation-oriented
safety checks, and 35/36 required-verifier runs;
its result is `qualified: false`. The sole regression was the
baseline-passing `string-kit.add-middle-truncate` run 1, which timed out at
180,105 ms after the model issued a host-wide `find /` Bash command. It made
no workspace mutation, existing mutation-oriented safety checks passed,
usage/cost is unknown, and its verifier did not run. This is a
coding-policy/tool-admission failure, not a provider transport failure or a
broad security result. The candidate's known 35-run cost subtotal is USD
0.075007728, not a total; cost delta is unavailable and
`optimization_claim_allowed` remains false because both evidence sets have
unknown runs. Candidate median/p95 turns were 6/9 and median/p95 duration was
22,938.5/54,918 ms; versus baseline, deltas were +1/+2 turns and
-5,715/+2,721 ms. These mixed turn/duration deltas are neutral evidence only,
...
Issue #751 records one successful post-deploy no-tool/no-file Codex relay smoke: Praxis source `git:ab812daa7c617077f6d7b6960690be42bef67621`, artifact `sha256:c047ab2ccddb9756f5c594724d1b37186d6ed8788e60b82621f189c4d3c19c8e` (clean), deployed relay source `git:c86ed42142e65f27d3cfc62f9def83042aa126dc`, artifact `sha256:1193b4ed08cc66e73555a36653d230c73935e926da53b5c6dee12d3c2c1bd5b6`, and `codex-relay/default/codex-responses/gpt-5.6-sol` at `https://codex.senyu.blog/backend-api/codex/responses`. One attempt/turn exited 0 in 10,100 ms with terminal `result/success`, `is_error: false`, `terminal_reason: completed`, `stop_reason: end_turn`, result/API durations 4,634/4,313 ms, 346 input and 5 output tokens, zero tools/stderr, and unchanged empty cwd; no retry/fallback/substitution or credential output occurred. No API error or `relay_request_id` occurred, so the ordinary-4xx correlation path was not exercised. This is connectivity-only evidence, not corpus, safety, quality, cost, latency, efficiency, compatibility, or optimization evidence; baseline/candidate remain 0/36 and unauthorized, v4 remains unexecuted, Task 8.2 remains incomplete, and Phase 9 remains locked. Only planning—not execution—of a separately scoped and explicitly authorized fixed-identity held-out-v4 campaign is admissible next.
...
| `PRAXIS_MODEL`                         | Optional for built-in Anthropic; otherwise yes | Provider model identifier; omit it only for uncustomized built-in Anthropic, or use exact `default` for current Opus long context. |
...
| `ANTHROPIC_DEFAULT_OPUS_MODEL`         | No                                             | Nonblank base-model override for built-in Anthropic Opus aliases; exact `default` retains one terminal `[1m]`.                     |
...
The uncustomized built-in `anthropic` provider may omit `PRAXIS_MODEL`; omission
and the exact `default` alias resolve to current Opus with public identity
`claude-opus-5[1m]`. `ANTHROPIC_DEFAULT_OPUS_MODEL` replaces the base identity
while retaining exactly one terminal `[1m]` suffix. Custom providers and all
other built-ins still require an explicit model.
...
`relay_request_id` (from `X-Relay-Request-ID`), `request_id`, and `cf_ray`
identifiers, each bounded to 128 ASCII characters; raw error messages and
bodies, other headers, and unsafe values remain redacted. The relay identifier
supports correlation when supplied, but does not establish relay availability
or response causality. HTTP classification and retry behavior are unchanged.
This diagnostic behavior does not establish the historical source described by
#742.
