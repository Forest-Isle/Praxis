...
behavior, mutation-oriented safety, verifier, usage, subscription-cost,
duration/latency, quality, security, compatibility, efficiency, optimization,
baseline, candidate, or qualification conclusion. Resolve this
settings-forwarding gap under a new change before any separately authorized
future campaign. Task 8.2 remains incomplete and Phase 9 stays locked.
Issue #733 now fixes only this configuration admission path: the default
Project Eval factory preserves the explicitly selected caller native config
root for provider definitions, selection, and credentials consistently across
...
Issue #748 is a separate authorized pre-corpus smoke against clean runtime
...
and one maximum turn. Provider-free gates passed (clean `npm ci` with zero vulnerabilities, clean build,
66 focused tests, one focused Project Eval regression, doctor 12/12, healthy
sanitized Worker v9 root, and valid eight-record catalog). Its sole smoke child
exited 1 after 3,350 ms with terminal `result/success`, `is_error: true`,
`terminal_reason: api_error`, HTTP 404, and bounded diagnostics
`type=invalid_request_error`, `code=model_not_found`, `cf_ray=a37a43e08dfd086a-YYZ`.
Result duration was 2,478 ms and API duration 0 ms. The one attempt had zero
tokens, tools, tool calls, stderr, and cwd entries; there was no signal, timeout,
overflow, mutation, or credential occurrence, and no request identifier.
stdout/stderr digests were `sha256:11abbf1e74138acd57311c3702ef5e6777165712c71d76503307be2556b051e2`
and `sha256:e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`.
No retry, substitution, or selection happened;
baseline/candidate authorizations were not admitted, markers/roots are absent,
and each remains 0/36. No verdict exists and v4 remains unexecuted. The diagnostic
is specific to #748's response, does not identify its producing layer, and does
not explain immutable #742. Smoke counters are not corpus usage/latency/cost
evidence; subscription cost is unavailable. Task 8.2 remains incomplete and
Phase 9 stays locked.
Issue #751 records fresh explicit user authorization for the newly pinned exact provider request, followed by provider-free admission: both Praxis tracked trees were clean at `git:ab812daa7c617077f6d7b6960690be42bef67621`; relay source was pinned to `git:c86ed42142e65f27d3cfc62f9def83042aa126dc`, tracked/deployed checksum matched `sha256:1193b4ed08cc66e73555a36653d230c73935e926da53b5c6dee12d3c2c1bd5b6`, the root-owned deployed file was mode 700, service was active/enabled with one 8088 listener, loopback/direct/edge health was `ok` for `codex-relay-v3` with 3/3 active accounts and zero shadow bans, native client-0.153.4 catalog returned eight models including the pin, `doctor --json` was 12/12, harness syntax passed, isolated cwd was empty, and build completed before the request. The attempt lock and named smoke/baseline/candidate/held-out-v4 output roots were absent before admission; the lock was then created exclusively for this one attempt.

Issue #751 records one successful post-deploy no-tool/no-file Codex relay smoke: Praxis source `git:ab812daa7c617077f6d7b6960690be42bef67621`, artifact `sha256:c047ab2ccddb9756f5c594724d1b37186d6ed8788e60b82621f189c4d3c19c8e` (`source_dirty: false`), deployed relay source `git:c86ed42142e65f27d3cfc62f9def83042aa126dc`, artifact `sha256:1193b4ed08cc66e73555a36653d230c73935e926da53b5c6dee12d3c2c1bd5b6`, provider/profile/protocol/model `codex-relay/default/codex-responses/gpt-5.6-sol`, endpoint `https://codex.senyu.blog/backend-api/codex/responses`. Configuration was high effort, 32,768 context, 4,096 reserve, provider-managed output, non-stream fallback disabled, 30/60/180-second clocks, zero tools, one maximum turn, no session persistence, slash commands disabled, and initially empty isolated cwd. One attempt/turn completed in 10,100 ms with exit 0, terminal `result/success`, `is_error: false`, `terminal_reason: completed`, `stop_reason: end_turn`, result/API durations 4,634/4,313 ms, 346 input and 5 output tokens, 2-byte response text, zero stderr, and unchanged empty cwd; no retry, fallback, substitution, signal, spawn error, outer timeout, overflow, or credential output occurred. Stdout was 960 bytes (`sha256:a8ec71c3b2af4303ef37505127b85f4df3b578ff7b625bd56e9da56f0ff707b5`), stderr empty (`sha256:e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`), and raw streams, prose, bodies, headers, accounts, and logs were not retained. No API error or `relay_request_id` occurred, so the ordinary-4xx correlation path was not exercised. This is connectivity evidence only, not corpus, safety, quality, cost, latency, efficiency, compatibility, or optimization evidence; baseline/candidate were neither authorized nor executed (0/36 each), v4 remains unexecuted, Task 8.2 remains incomplete, and Phase 9 remains locked. The next admissible frontier is planning—not executing—a separately scoped and explicitly authorized fixed-identity held-out-v4 campaign; no v4 task, fixture detail, verifier, result aggregate, comparison, or qualification verdict was read or produced.

### Task 8.2: Real-model qualification [depends: Task 8.1]

Issue #748's failed-closed gpt-5.5 pre-corpus smoke did not admit baseline or
candidate (both 0/36 with absent outputs), so this task remains incomplete;
v4 was not executed and Phase 9 remains locked. Its bounded `model_not_found`
diagnostic applies only to that response and does not explain immutable #742.
...
pinned `openai-responses/default/openai-responses/gpt-5.5` configuration through
...
relay-outage, request-shape, or edge-route cause is claimed. The predeclared
