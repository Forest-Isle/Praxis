...
| Stream JSON output                   | Partial                  | Init, assistant, tool result, result, warning, compaction status/boundary, session state, tool progress, task lifecycle, API retry, error, measured result metrics, and `prompt_suggestion` records are wired to real local events; Claude 2.1.208 `/cost` emits a synthetic assistant, not `local_command_output`; `tool_use_summary`, files-persisted, and elicitation still have no observed local producer |
