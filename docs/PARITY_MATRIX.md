...
| Single JSON result                   | Partial                  | SDK success/error subtype, UUID, stop reason, error list, fast-mode state, plugin-aware init, measured API duration, and priced usage; unknown model pricing/API failure duration remain explicitly `null`                                                                                                                                                                                                                               |
| Stream JSON output                   | Partial                  | Init, assistant, tool result, result, warning, compaction status/boundary, session state, tool progress, task lifecycle, API retry, error, measured result metrics, `prompt_suggestion`, MCP elicitation control/completion, and provider-backed `tool_use_summary` records are wired to real local events; Claude 2.1.208 `/cost` emits a synthetic assistant, not `local_command_output`; remote-only files-persisted remains excluded |
