...
| Stream JSON output                   | Partial                  | Init, assistant, tool result, result, warning, compaction status/boundary, session state, tool progress, task lifecycle, API retry, error, measured result metrics, `prompt_suggestion`, and MCP elicitation control/completion records are wired to real local events; Claude 2.1.208 `/cost` emits a synthetic assistant, not `local_command_output`; `tool_use_summary` and files-persisted still have no observed local producer |
