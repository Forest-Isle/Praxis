...
| Stream JSON output                   | Complete for local CLI   | Init, assistant, tool result, result, warning, compaction status/boundary, session state, tool progress, task lifecycle, API retry, error, measured result metrics, `prompt_suggestion`, MCP elicitation control/completion, and provider-backed `tool_use_summary` records are wired to real local events; 2.1.208 maps local-command output to assistant records; remote-only files-persisted is excluded |
...
| Workflow                           | Partial  | Runtime, sandbox, task lifecycle, native files/options, Claude-created unchanged-script/args ordered replay, Praxis semantic sidecar replay, and live Claude -> Praxis zero-child resume complete; exact Praxis -> Claude replay key unknown |
